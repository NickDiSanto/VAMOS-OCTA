...
def gradient_magnitude(slice_2d):
    """Approximate per-pixel gradient magnitude for a 2D image."""
...
def to_lpips_tensor(x):
    """Convert a 2D [0, 1] array to LPIPS tensor format `(1, 3, H, W)` in [-1, 1]."""
...
def edge_strength(im):
    """Aggregate Sobel edge energy across an image."""
    dx = sobel(im, axis=0)
    dy = sobel(im, axis=1)
...
def laplacian_blur_score(image):
    """Compute variance of the Laplacian; lower values indicate blurrier images."""
...
def compute_psnr(gt, pred, data_range=1.0):
    """Compute PSNR using the same convention as the evaluation scripts."""
