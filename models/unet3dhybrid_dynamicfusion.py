...
class ModulatedInputConv(nn.Module):
    """Dynamically rescale a base convolution using the slice-validity pattern."""
    ...
    def forward(self, x, validity_mask):
        ...
        Args:
            ...
            validity_mask: Tensor of shape (B, in_channels).
        """
        batch_size, _, _, _ = x.shape

        scales = self.scale_fc(validity_mask)
        scales = scales.view(batch_size, self.out_channels, self.in_channels)

        weight = self.base_conv.weight
        outputs = []
        for batch_idx in range(batch_size):
            scaled_weight = scales[batch_idx].unsqueeze(-1).unsqueeze(-1) * weight
            outputs.append(F.conv2d(x[batch_idx].unsqueeze(0), scaled_weight, padding=1))

        return F.relu(torch.cat(outputs, dim=0))


class UNet3DHybrid_DynamicFusion(nn.Module):
    """
    Hybrid 3D-2D U-Net variant for slice-wise inpainting.

    This design encodes the stacked volume with 3D convolutions, collapses the depth
    axis at the bottleneck, and uses the center slice from each encoder stage as the
    skip feature passed into the 2D decoder.

    Args:
        in_channels (int): Number of slices per input stack.
        ...
        stack_size (int): Number of slices in the input stack.
    """
    ...
    def forward(self, x):
        ...
        """
        _, num_slices, _, _ = x.shape
        x_3d = x.unsqueeze(1)

        x1 = self.encoder1(x_3d)
        x2 = self.encoder2(self.pool1(x1))
        x3 = self.encoder3(self.pool2(x2))
        x3_pooled = self.pool3(x3)
        x_bottleneck = self.axial_collapse(x3_pooled).squeeze(2)
        x = self.dropout(x_bottleneck)

        # Each skip path uses the center slice feature map from the corresponding 3D stage.
        x = self.up1(x, x3[:, :, num_slices // 2])
        x = self.dropout(x)
        x = self.up2(x, x2[:, :, num_slices // 2])
        x = self.dropout(x)
        x = self.up3(x, x1[:, :, num_slices // 2])
        x = self.dropout(x)
        return self.outc(x)
